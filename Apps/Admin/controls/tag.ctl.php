...
class TagController extends SmcmsController {
    ...
    public function indexAction() {
        ...
        if($getid > 0){
            $id = $getid;
            $rows = DB::getlist("select * from @pf_tag where pid=? and id = ? and `type`=? order by sort asc",array(0,$id,$type));
        }else{
            $rows = DB::getlist("select * from @pf_tag where pid=? and `type`=? order by sort asc",array(0,$type));
        }//ysq
