...
class Wxpay {
     ...
     */
    function wxRefund($outTradeNo,$outRefundNo,$refundAmount,$totalAmount)
    {
        $merchid = WxPayConfig::MCHID;
        //var_dump(WxPayConfig::SSLCERT_PATH);

        $input = new WxPayRefund();
        $input->SetOut_trade_no($outTradeNo);            //自己的订单号
        $input->SetOut_refund_no($outRefundNo);            //自己的订单号
        $input->SetTotal_fee($totalAmount*100);            //订单标价金额，单位为分
        $input->SetRefund_fee($refundAmount*100);            //退款总金额，订单总金额，单位为分，只能为整数
