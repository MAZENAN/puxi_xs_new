...
//        }
        if ($this->upType == 'upload') {
            //移动文件
            $rm = $this->uploadAliyun($this->file['tmp_name'],$this->fileName);
            if ($rm) {
...
//        }
    ...
    public function uploadAliyun($file , $filename)
    { 
        require_once ROOT_DIR . 'libs/aliyun/samples/Common.php';

        $ossClient = Common::getOssClient();
        if (!is_null($ossClient)) {
             // Uploads a local file to an OSS file
            $result = $ossClient->uploadFile(C('aliyunOss')['bucket'], $filename,$file);
