...
(function ($, undefined) {
    ...
    var initAjaxUpFile = function (box, options) {
        ...
        var storageRoot ="http://puxi-public1.oss-cn-beijing.aliyuncs.com/";
        console.log(returnbox.val())
        var isinput = returnbox.is('input');
        if (isinput) {
            var reval = storageRoot+returnbox.val();
            returnbox.attr('ref', storageRoot+returnbox.val());
            if (bindimg) {
        ...
        var uploadCompleteData = function (data) {
            ...
            if (data && data.err === '' && !noSetVal) {
                ...
                if (isinput) {
                    ...
                    returnbox.attr('ref',storageRoot+data.msg.url);
                }
                if (bindbox) {
